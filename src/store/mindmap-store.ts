...
export const useMindMapStore = create<MindMapState>((set, get) => ({
  ...
  addNode: (partial) => {
    // Enforce the configurable maxNodes limit (Settings → Performance).
    // Previously the setting existed but was never consulted, so maps could
    // grow unbounded and freeze the canvas. We now refuse to add beyond the
    // limit and surface a console warning (the UI layers toast on their own
    // when addNode returns an empty-ish id, but here we still return a valid
    // id for the node that DID get created when under the limit).
    const maxNodes = useSettingsStore.getState().settings.performance.maxNodes;
    const currentCount = get().nodes.length;
    if (maxNodes > 0 && currentCount >= maxNodes) {
      // Soft-enforce: skip creation and return an empty string so callers
      // can detect the no-op. Log for debugging.
      if (typeof console !== "undefined") {
        console.warn(`[mindmap] Limite de nós atingido (${currentCount}/${maxNodes}). addNode ignorado.`);
      }
      return "";
    }
  ...
  duplicateNode: (id) => {
    ...
    if (!src) return null;
    // Snapshot current state so duplication is undoable as a single step.
    get().pushHistory();
    const newId = uid("n");
    const copy: MapNode = {
      ...src,
    ...
    // Also duplicate incoming edges so the copy keeps its visual relationships
    // to existing neighbours (edges whose source/target was the original node).
    const edgeClones: MapEdge[] = get()
      .edges.filter((e) => e.sourceId === id || e.targetId === id)
      .map((e) => ({
        ...e,
        id: uid("e"),
        sourceId: e.sourceId === id ? newId : e.sourceId,
        targetId: e.targetId === id ? newId : e.targetId,
        createdAt: new Date().toISOString(),
      }));
    set((s) => ({
      nodes: [...s.nodes, copy],
  ...
  focusNode: (id) => {
    ...
    if (!node) return;
    // center on node — preserve the current zoom so adding/focusing a node
    // does not abruptly reset the viewport scale (which was jarring).
    const cx = typeof window !== "undefined" ? window.innerWidth / 2 : 600;
    const cy = typeof window !== "undefined" ? window.innerHeight / 2 : 400;
    const zoom = get().viewport.zoom;
    set({
      viewport: {
        x: cx - (node.x + node.width / 2) * zoom,
        y: cy - (node.y + node.height / 2) * zoom,
        zoom,
      },
      selectedNodeIds: [id],
    });
  ...
  toggleCollapse: (id) => {
    // Snapshot so collapse/expand is undoable.
    get().pushHistory();
    set((s) => ({
      nodes: s.nodes.map((n) =>
  ...
  organizeLayout: () => {
    ...
    // Snapshot current positions so auto-organize is a single undoable step.
    get().pushHistory();
    ...
    for (let iter = 0; iter < iterations; iter++) {
      for (let i = 0; i < nodes.length; i++) {
        ...
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          // BUGFIX: the previous expression was parsed as
          //   ((centerA) - posB.x) + halfB  due to left-to-right precedence,
          // producing wrong overlap detection. Correctly compare centers:
          const centerAx = posA.x + nodes[i].width / 2;
          const centerBx = posB.x + nodes[j].width / 2;
          const centerAy = posA.y + nodes[i].height / 2;
          const centerBy = posB.y + nodes[j].height / 2;
          const overlapX = wA / 2 + wB / 2 - Math.abs(centerAx - centerBx);
          const overlapY = hA / 2 + hB / 2 - Math.abs(centerAy - centerBy);
